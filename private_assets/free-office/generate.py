...
(p/'manifest.json').write_text(json.dumps({'id':'woodland-office','version':'1.0.0','name':'Woodland Office','license':'CC0-1.0','palette':{'wood':wood,'frame':dark,'trim':trim,'fabric':sage,'cushion':light,'screen':ink},'families':list(art)},indent=2)+'\n')
(p/'preview.svg').write_text('<svg xmlns="http://www.w3.org/2000/svg" width="768" height="320" viewBox="0 0 768 320"><rect width="768" height="320" fill="#eee5cc"/>'+''.join(f'<g transform="translate({(i%6)*128+32} {(i//6)*150+14})">{body}<text x="32" y="88" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#344c47">{name}</text></g>' for i,(name,body) in enumerate(art.items()))+'</svg>')
