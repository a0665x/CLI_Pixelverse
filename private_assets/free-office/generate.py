...
for name,body in art.items():
 (p/'sprites'/f'{name}.svg').write_text(f'<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g stroke="#25272b" stroke-width="2.2" stroke-linejoin="round" paint-order="stroke fill">{body}</g></svg>')
...
(p/'preview.svg').write_text('<svg xmlns="http://www.w3.org/2000/svg" width="768" height="470" viewBox="0 0 768 470"><rect width="768" height="470" fill="#eee5cc"/>'+''.join(f'<g transform="translate({(i%6)*128+32} {(i//6)*150+14})"><g stroke="#25272b" stroke-width="2.2" stroke-linejoin="round" paint-order="stroke fill">{body}</g><text x="32" y="88" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#344c47">{name}</text></g>' for i,(name,body) in enumerate(art.items()))+'</svg>')
