...
const divider = (id: string, assetId: number, point: GridPoint, facing: Facing, supportedByIds: string[]) => (
  furniture({
    id, kind: 'cabinet', assetId, point, facing, layer: 'surface', zIndex: 5, blocksNavigation: false, supportedByIds,
  })
);

const surface = (
  id: string, kind: FurnitureKind, assetId: number, point: GridPoint, facing: Facing, supportedByIds: string[],
) => (
  furniture({ id, kind, assetId, point, facing, layer: 'surface', zIndex: 20, blocksNavigation: false, supportedByIds })
);
...
const defineOfficePrefab = (input: PrefabInput): OfficePrefabDefinition => {
  ...
  for (const item of prefab.items) {
    ...
    if (item.supportedByIds) Object.freeze(item.supportedByIds);
    Object.freeze(item);
  }
...
const benchFour = defineOfficePrefab({
  ...
  items: [
    ...
    divider('bench-divider-horizontal', 208, { x: 0, y: 2 }, 'right', [
      'bench-desk-nw', 'bench-desk-ne', 'bench-desk-sw', 'bench-desk-se',
    ]),
    divider('bench-divider-cross', 207, { x: 0.125, y: 1 }, 'down', [
      'bench-desk-nw', 'bench-desk-ne', 'bench-desk-sw', 'bench-desk-se',
    ]),
    surface('bench-monitor-nw', 'display', 121, { x: 0, y: 1 }, 'down', ['bench-desk-nw']),
    surface('bench-monitor-ne', 'display', 121, { x: 1, y: 1 }, 'down', ['bench-desk-ne']),
    surface('bench-monitor-sw', 'display', 125, { x: 0, y: 2 }, 'up', ['bench-desk-sw']),
    surface('bench-monitor-se', 'display', 125, { x: 1, y: 2 }, 'up', ['bench-desk-se']),
    chair('bench-chair-nw', 105, { x: 0, y: 0 }, 'down'),
    chair('bench-chair-ne', 105, { x: 1, y: 0 }, 'down'),
    chair('bench-chair-sw', 101, { x: 0, y: 3.5 }, 'up'),
    chair('bench-chair-se', 101, { x: 1, y: 3.5 }, 'up'),
    surface('bench-keyboard-nw', 'decor', 124, { x: 0, y: 1 }, 'down', ['bench-desk-nw']),
    surface('bench-keyboard-ne', 'decor', 124, { x: 1, y: 1 }, 'down', ['bench-desk-ne']),
    surface('bench-papers-sw', 'decor', 153, { x: 0, y: 2 }, 'up', ['bench-desk-sw']),
    surface('bench-lamp-se', 'decor', 141, { x: 1, y: 3 }, 'up', ['bench-desk-se']),
  ],
  interactionAnchors: [
...
const podLTwo = defineOfficePrefab({
  ...
  items: [
    ...
    divider('pod-divider', 207, { x: 0, y: 1 }, 'right', ['pod-desk-north', 'pod-desk-west', 'pod-desk-east']),
    surface('pod-monitor-west', 'display', 129, { x: 0, y: 2 }, 'right', ['pod-desk-west']),
    surface('pod-monitor-east', 'display', 131, { x: 1, y: 1 }, 'up', ['pod-desk-east']),
    chair('pod-chair-west', 103, { x: 1, y: 2.5 }, 'left'),
    chair('pod-chair-east', 105, { x: 1, y: 0 }, 'down'),
    surface('pod-keyboard-west', 'decor', 124, { x: 0, y: 2 }, 'right', ['pod-desk-west']),
    surface('pod-keyboard-east', 'decor', 128, { x: 1, y: 1 }, 'up', ['pod-desk-east']),
    surface('pod-papers', 'decor', 155, { x: 0, y: 1 }, 'down', ['pod-desk-north']),
...
const controlMThree = defineOfficePrefab({
  ...
  anchor: { x: 0, y: 0 },
  items: [
    desk('control-desk-left', 267, { x: 0, y: -0.25 }, 'down', controlActions, 'tool'),
    desk('control-desk-center', 267, { x: 2, y: -0.25 }, 'down', controlActions, 'tool'),
    desk('control-desk-right', 267, { x: 4, y: -0.25 }, 'down', controlActions, 'tool'),
    desk('control-wing-left', 266, { x: 1, y: 1 }, 'left', controlActions, 'tool'),
    desk('control-wing-right', 266, { x: 3, y: 1 }, 'right', controlActions, 'tool'),
    divider('control-divider-left', 207, { x: 1, y: 1 }, 'up', ['control-wing-left']),
    divider('control-divider-right', 207, { x: 3, y: 1 }, 'up', ['control-wing-right']),
    surface('control-monitor-left', 'display', 129, { x: 0, y: -0.25 }, 'down', ['control-desk-left']),
    surface('control-monitor-center', 'display', 133, { x: 2, y: -0.25 }, 'down', ['control-desk-center']),
    surface('control-monitor-right', 'display', 131, { x: 4, y: -0.25 }, 'down', ['control-desk-right']),
    surface('control-keyboard-left', 'decor', 124, { x: 0, y: -0.25 }, 'down', ['control-desk-left']),
    surface('control-keyboard-center', 'decor', 128, { x: 2, y: -0.25 }, 'down', ['control-desk-center']),
    surface('control-keyboard-right', 'decor', 124, { x: 4, y: -0.25 }, 'down', ['control-desk-right']),
    chair('control-chair-left', 101, { x: 0, y: 1 }, 'up'),
    chair('control-chair-center', 101, { x: 2, y: 1 }, 'up'),
    chair('control-chair-right', 101, { x: 4, y: 1 }, 'up'),
    surface('control-documents', 'decor', 155, { x: 2, y: -0.25 }, 'down', ['control-desk-center']),
  ],
  interactionAnchors: [
