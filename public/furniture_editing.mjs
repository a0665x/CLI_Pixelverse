...
export const SCALE_BOUNDS = Object.freeze({ min: 0.55, max: 1.8 });
...
export function clampScale(value, bounds = SCALE_BOUNDS) {
  const numeric = Number.isFinite(Number(value)) ? Number(value) : 1;
  return Math.max(bounds.min, Math.min(bounds.max, numeric));
}

export function normalizeScale(value, step = SCALE_STEP) {
  const safeStep = Number(step) > 0 ? Number(step) : SCALE_STEP;
  return Number((Math.round(clampScale(value) / safeStep) * safeStep).toFixed(2));
}
